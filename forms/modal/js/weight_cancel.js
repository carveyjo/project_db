$(document).ready(function() {
  ...
  $("#cancel").submit(function(event) {
    ...
    $.ajax({
      ...
      url: '/forms/modal/php/weight_cancel.php',
  ...
  function showSubmitData(KeyId, success, e) {
    if (success == true) { //If successful ---
      tableLine = '<tr KeyId="addedrow"><th>' + counter + '</th>';
      tableLine += "<th class='alert-success'>Success</th></tr>"; //Show Green
    } else { //If error ---------------------
      tableLine = "<tr KeyId='addedrow'><th class='alert-danger'>Error</th><th class='alert-danger'>" + KeyId + '</th>'; //Know error duplicate \/                                                        //Unknown error// --Generic
  ...
  function raiseAlert(KeyId, success) {
    if (success == true) { //If successful---------
      ...
      if (id != "none") { //if KeyId is given - (known error) \/ enter text
